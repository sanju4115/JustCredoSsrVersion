...
export default {
  validateClassesType: function(classesType){
    if (classesType.length <= 0) {
      return {
        valid: false,
        errorText: "Please select at least one from class available.",
        errorSnackbar: true
      };
    }else {
      return {
  ...
  validateGenres: function(
    ...
    let selectType = schoolType.join(".");
    if (selectType.includes("Singing") && singingClass.length <= 0) {
      return {
        valid: false,
        errorText: "Please select at least one singing list.",
        errorSnackbar: true
      };
    } else if (selectType.includes("Dance") && danceClass.length <= 0) {
      return {
        valid: false,
        errorText: "Please select at least one dance list.",
        errorSnackbar: true
      };
    } else if (
      selectType.includes("Instruments") &&
      instrumentsClass.length <= 0
    ) {
      return {
        valid: false,
        errorText: "Please select at least one instruments list.",
        errorSnackbar: true
      };
    } else {
      return {
  ...
  validateFacilityAndExtraCurricullar: function(
    ...
    if (
      ...
      !(entityType === EntityConstants.MUSIC_CLASS || entityType === EntityConstants.SPORTS_CLASS)
    ) {
      return {
        valid: false,
  ...
  validateTypeAndGender: function(schoolType, genderList, entityType) {
    if (schoolType.length <= 0 && (entityType !== EntityConstants.SPORTS_CLASS )) {
      return {
        valid: false,
