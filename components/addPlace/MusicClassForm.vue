    ...
    <v-flex xs12 v-else>
      <v-stepper v-model="schoolForm" vertical>
        ...
        <v-stepper-content step="3">
          <v-card class="ma-2">
            <MusicClassInput
              :dance-class="formData.dancing"
              :instruments-class="formData.singing"
              :singing-class="formData.instruments"
              :dance-class-selected="danceClass"
              :singing-class-selected="singingClass"
              :instruments-class-selected="instrumentsClass"
              @change-danceClass="danceClass = $event"
              @change-singingClass="singingClass = $event"
              @change-instrumentsClass="instrumentsClass = $event"></MusicClassInput>
          </v-card>
  ...
  methods: {
    ...
    validation() {
      ...
      if (this.schoolForm === 1) {
        ...
        const result = SchoolFormValidation.validateGenres(
          this.schoolType,
          this.singingClass,
          this.danceClass,
          this.instrumentsClass
        );
        if (result.valid) {
          this.schoolForm = 4;
        } else {
